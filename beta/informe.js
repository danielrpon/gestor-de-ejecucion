...
async function generarPPTX(m, lista, fotos, op, parte){
  ...
  s.addText([{text:"INFORME TÉCNICO DE EJECUCIÓN",options:{fontSize:30,bold:true,color:AZUL,breakLine:true}},
             {text:recorta(nombre,70),options:{fontSize:22,bold:true,color:"222222",breakLine:true}},
             {text:(m.ev.cl||"")+(m.cfg.fe?" · "+cap(rango(m.cfg.fe,m.cfg.ff).replace(/^(el|del) /,"")):""),options:{fontSize:15,color:GRIS,breakLine:true}},
             {text:`OPERACIÓN LOGÍSTICA · ${op.operador.toUpperCase()}${tituloParte}`,options:{fontSize:12,color:GRIS}}],
    {x:0.55,y:3.85,w:6.6,h:2.65,fontFace:F,valign:"top",margin:0,paraSpaceAfter:4});
  if(op.version==="interno") s.addText("CONTROL INTERNO · no enviar al cliente",{x:0.55,y:6.55,w:6,h:0.35,fontFace:F,fontSize:11,bold:true,color:ROJO,margin:0});
  ...
  if(parte.n===1){
    ...
    if(op.hojasPegar){
      [["Requerimiento "+(m.ev.cl||"del cliente"),"Pegue aquí la imagen del requerimiento del cliente"],["Cotización operador","Pegue aquí la imagen de la cotización"]].forEach(([t,h])=>{
        ...
        s.addShape(pptx.ShapeType.rect,{x:1.2,y:1.4,w:10.9,h:5.2,fill:{color:"F7F9FC"},line:{color:"9FB0C8",width:1.25,dashType:"dash"}});
        s.addText(h,{x:1.2,y:3.6,w:10.9,h:0.6,align:"center",fontFace:F,fontSize:16,color:"7A8BA3"}); });
    }
  }

  // 4 · Evidencias por requerimiento
  const COLS=4, GAP=0.22, MX=0.5, Y0=1.3, ALTO=5.45, PIE=0.32;
  ...
  if(parte.n===parte.total){
    ...
    s.addText(P.map((t,k)=>({text:t,options:{breakLine:k<P.length-1,paraSpaceAfter:10}})),{x:1.0,y:1.45,w:11.3,h:4.3,fontFace:F,fontSize:16,color:"222222",valign:"top",margin:0,fit:"shrink"});
    ...
    if(N.length){ s=pptx.addSlide({masterName:"EST"}); cabecera(s,"Novedades de cobertura (control interno)",`${N.length} novedad(es)`);
      s.addText(N.slice(0,14).map((t,k)=>({text:t,options:{bullet:true,breakLine:k<Math.min(N.length,14)-1}})),{x:0.9,y:1.4,w:11.6,h:4.9,fontFace:F,fontSize:13,color:"222222",valign:"top",margin:0});
      if(N.length>14) s.addText(`… y ${N.length-14} más (ver el resumen de cobertura).`,{x:0.9,y:6.35,w:11,h:0.35,fontFace:F,fontSize:11,italic:true,color:GRIS,margin:0}); }
